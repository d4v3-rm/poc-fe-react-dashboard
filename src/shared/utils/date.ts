...
export const isWithinNextWeek = (isoDate: string | null): boolean => {
  ...
  return due.isAfter(today.subtract(1, 'millisecond')) && due.isBefore(endWindow.add(1, 'millisecond'));
};

export const formatDueDate = (isoDate: string | null, language: 'en' | 'it'): string => {
  if (!isoDate) {
    return language === 'it' ? 'Nessuna scadenza' : 'No due date';
  }

  return dayjs(isoDate).locale(language).format(language === 'it' ? 'DD MMM YYYY' : 'MMM D, YYYY');
};
